...
async function startServer() {
  ...
  app.post("/api/db/query", async (req, res) => {
    try {
      ...
      const { table, select = "*", filters = [], order, limit, page = 1 } = req.body;
      if (!table) {
  ...
  app.post("/api/db/insert", async (req, res) => {
    try {
      ...
      const query = upsert ? supabase.from(table).upsert(records).select() : supabase.from(table).insert(records).select();
  ...
  app.delete("/api/db/delete", async (req, res) => {
    try {
      ...
      if (id !== void 0) {
        ...
        return res.status(400).json({ error: 'Provide either "id" or a "match" object to delete.' });
      }
      const { data, error } = await query.select();
      if (error) {
        return res.status(400).json({
